...
ifstream jury, user, expected_ans;
map<int,int> vertice[N];
vector<int> user_vec;
void input_testcase() {
    jury >> n >> m;
    int x, y;
...
void run_custom_checker(const TestStruct t_obj,
    ...
    user.open(t_obj.testcase_output_path);
    jury.open(t_obj.testcase_input_path);
    expected_ans.open(t_obj.testcase_expected_output_path);
    
    expected_ans >> n_ans;     
    user >> n;
    if (n_ans != n) {
        r_obj.message = "Shortest";
        return;
    }
    input_testcase();
    input_user();
    ...
    for (int i = 0; i < user_vec.size() - 1; i++) {
        int x = user_vec[i], y = user_vec[i + 1];
        if (vertice[x][y] != 1) check = false;
    }
    if (user_vec[user_vec.size()-1] != n) check = false;
